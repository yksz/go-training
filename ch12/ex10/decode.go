...
func init() {
	registerType(reflect.TypeOf(0))       // int
	registerType(reflect.TypeOf([]int{})) // []int
}

func registerType(t reflect.Type) {
	typemap[fmt.Sprintf("\"%s\"", t.String())] = t
}
...
func readList(lex *lexer, v reflect.Value) {
	...
	case reflect.Interface: // (type value)
		...
		if !ok {
			panic(fmt.Sprintf("not support type %s", lex.text()))
		}
		inf := reflect.New(typ).Elem()
		lex.next()
		read(lex, inf)
		v.Set(inf)
